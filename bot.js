        ...
        for (let i = 1; i <= practiceCount; i++) {
            ...
            if (threeDaysBefore > new Date()) {
                notifications.push({
                    ...
                    message: this.createThreeDayMessage(name, i, practiceDeadline, endDate)
                });
            }
        }
...
Команда и проводник могут ориентироваться на этот чекпоинт для планирования 🌱`;
    ...
    createFinalReminderMessage(clientName, finalDeadline) {
        return `🌟 Важное напоминание о финальном сроке разработки

${clientName}, через 3 дня наступает финальный срок разработки - ${this.formatDate(finalDeadline)}.

💎 Это ключевая дата завершения всего пути. В отличие от промежуточных ориентиров, этот срок является важным рубежом.

🤗 Понимаем, что каждый работает в своём ритме. Команда и проводник всегда готовы поддержать на финальном этапе.

💫 Если нужна помощь или есть вопросы - обращайтесь. Мы верим в успешное завершение!

С теплотой и поддержкой всей команды 🌸`;
    }
